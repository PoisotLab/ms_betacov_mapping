...
patterns. Even accounting for these potential biases, though, there are
obvious and distinctive regional regimes of bat-betacoronavirus
diversification.

These global patterns recapitulate the evolutionary history of both the
order Chiroptera and the genus \emph{Betacoronavirus}. Horseshoe bats
(Rhinolophidae) are both the reservoirs of the SARS-like viruses
(subgenus \emph{Sarbecovirus}) and the presumed ancestral host of
...
richness---is almost entirely driven by dynamics in this
clade.\textsuperscript{3,14} In contrast, the Neotropical hotspot is
driven by a combination of distinct bat and virus adaptive radiation.
The hotspot of bat diversity in the Americas is driven by the New World
leaf-nosed bats (Phyllostomidae),\textsuperscript{18} a group of bats
that have undergone multiple explosive diversification events--- but
that are more often hosts of alphacoronaviruses. Out of the four main
groups of betacoronaviruses, only the subgenus \emph{Merbecovirus}---the
outgroup of the betacoronavirus tree---has been found in New World bats;
while found worldwide, the merbecoviruses are almost entirely restricted
to vesper bats (Vespertilionidae) {[}{]}. In this light, the hotspot of
virus distinctiveness in the Americas is the result of isolation by
vicariance--- a unique branch of coronavirus-bat coevolution, separate
...
congruence.\textsuperscript{19,20} In particular, this should be the
...
species.\textsuperscript{21} Immune characteristics that allow bats to
be better adapted to infection by emerging
viruses,\textsuperscript{22,23} in addition to being hardcoded in their
genome,\textsuperscript{24} may be related to a wide variety of
diets,\textsuperscript{21,25} themselves likely to be driven by spatial
...
environments.\textsuperscript{26}
...
human exposure to SARS-like viruses;\textsuperscript{27} MERS-CoV in
Saudi Arabia based on index cases available from a recently-published
compendium of cases.\textsuperscript{28} For the latest event, most if
...
Haiti\textsuperscript{29} and Europe\textsuperscript{30} are not
...
continents.\textsuperscript{31} Recent surveillance\textsuperscript{32}
...
widely.\textsuperscript{33} Since human activity is required to amplify
...
looking for the origins of SARS in China,\textsuperscript{34} present
...
health.\textsuperscript{19,20} Chiropterans emerged around 64 million
years ago and are one of the most diverse mammalian orders, with an
estimated richness of more than 1400 species.\textsuperscript{35,36}
...
ecosystem-derived benefits to human.\textsuperscript{37} For example,
bats are an essential component of many seed-dispersal
networks.\textsuperscript{38} Over two-thirds of bats are know to be
either obligate or facultative insectivores, therefore actively
contributing for agricultural pest control,\textsuperscript{39,40} and
vectors of pathogens that put a risk on human
health.\textsuperscript{41,42} Because bats are globally distributed and
...
lineage).\textsuperscript{43} Both of these regions are priority areas
...
novel diseases,\textsuperscript{47} so does the changes in landscape
connectivity at local/regional scales.\textsuperscript{48} This
...
encounters.\textsuperscript{49} In particular, the challenge ahead lies
...
data.\textsuperscript{50} The range maps were subsequently rasterized
using the \texttt{rasterize} function from
\texttt{GDAL}\textsuperscript{51} at a resolution of approximately
...
Diversity\textsuperscript{52} based on a recent synthetic tree with
robust time calibration, covering about 6000 mammalian
species.\textsuperscript{53} Faith's PD measures the sum of unique
...
Beta-Diversity;\textsuperscript{54} LCBD works from a species-data
...
location, and following best practices\textsuperscript{55} transformed
...
community. Recent results by\textsuperscript{56} shows that LCBD
...
mammals.\textsuperscript{57} This network stores pairwise values of
...
model.\textsuperscript{58} In this approach, every risk component gets
...
and low viral sharing,\textsuperscript{59} in that this allows multiple
...
extraction,\textsuperscript{60} we measured the risk level by
...
MAFFT\textsuperscript{61} v1.4.0 (Algorithm FFT-NS-2, Scoring matrix
200PAM / k=2, gap open penalty 1.53m offset value 0.123) and a maximum
likelihood tree reconstructed in IQ-TREE\textsuperscript{62} v1.6.12
with ModelFinder\textsuperscript{63} ultrafast bootstrap
approximation\textsuperscript{64} with a general time reversible model
...
distinctiveness\textsuperscript{65} for each of the viruses in the tree,
...
this idea, we loosely adapted a method from,\textsuperscript{66,67} who
...
cophenetic function in \texttt{ape};\textsuperscript{68} subsequently,
