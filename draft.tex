...
\hypertarget{the-coevolutionary-mosaic-generates-emergence-regimes}{%
\subsection{The coevolutionary mosaic generates emergence
regimes}\label{the-coevolutionary-mosaic-generates-emergence-regimes}}
...
(2) \emph{host community uniqueness}: coevolving with more unique host
communities should create more unique branches of viral evolution; and
(3) propensity for \emph{viral sharing}: frequent cross-species
transmission may act as a buffer on selective pressure, while lower
rates of exchange may enable more simultaneous trajectories of viral
specialization to coexist within a given community. We combine global
maps of all three to generate a map of coevolutionary regimes, where
close colors represent similar risks, and paler pixels represent overall
higher risk (fig.~\ref{fig:trivariate}). We find that these regions do
not neatly overlap with those defined in fig.~\ref{fig:richness} or
...
continents,\textsuperscript{24} is one such potential future hotspot.
Indeed, recent surveillance\textsuperscript{25} has identified a novel
