...
\raggedright
\thispagestyle{empty}
{\Large\bfseries\sffamily The coevolutionary mosaic of betacoronavirus
emergence risk}
\vskip 5em
...
co-divergence with hosts {[}Geoghegan{]}. This creates a disconnect
...
The Geographic Mosaic Theory of Coevolution\textsuperscript{GMTC; 6}
...
that coevolutionary processes among pairs\textsuperscript{7} or
complexes\textsuperscript{8} of species are structured in space by the
...
ecologically dynamics over large spatial extents.\textsuperscript{see
\emph{e.g.} 9} The GMTC predicts a spatial fragmentation of
coevolutionary dynamics under the joint action of three
processes:\textsuperscript{see notably 10} coevolutionary hot- and
coldspots, which appear when the intensity of \emph{interaction} (in
terms of reciprocal fitness consequences) varies spatially; selection
mosaics, wherein the intensity of \emph{selection} varies across space,
driven by both the biotic complexity of the community (locally diverse
hosts and viruses are more biotically complex) and the local
favorability of the environment;\textsuperscript{11} and trait remixing,
which occurs when coevolutionary dynamics are driven by by the arrival
(or departure) of \emph{functional traits}, through changes in community
composition due to invasions, meta-community dynamics, and disperal.
...
betacoronaviruses.\textsuperscript{12} This has attracted attention to
...
health.\textsuperscript{13,14} By overlaying the IUCN range maps for
confirmed bat hosts of betacoronaviruses {[}fig.~\ref{fig:richness};
top{]}, we see that the the main hotspots (both in terms of size and
higher values) of host richness are primarily South-Eastern Asia, parts
of Southern Europe, and to a lesser extent parts of Africa in the -25-0
range of latitudes. The description of host richness is an important
first step towards understanding risk, as previous
research\textsuperscript{2,15} states that locally diverse bat
...
the creation of novel interactions,\textsuperscript{16} and therefore
...
Yinpterochiroptera elsewhere.\textsuperscript{17,18} Specifically, we
...
the Phyllostomidae,\textsuperscript{19} which is restricted to the
...
global predictions by\textsuperscript{20} highlight that southeast Asia
...
discovered,\textsuperscript{2,21,22} resulting in sparser phylogenetic
...
that underwent explosive diversification events,\textsuperscript{19}
...
congruence.\textsuperscript{23,24} In particular, this should be the
...
species.\textsuperscript{25} Immune characteristics that allow bats to
be better adapted to infection by emerging
viruses,\textsuperscript{26,27} in addition to being hardcoded in their
genome,\textsuperscript{28} may be related to a wide variety of
diets,\textsuperscript{25,29} themselves likely to be driven by spatial
...
environments.\textsuperscript{30}
...
human exposure to SARS-like viruses;\textsuperscript{31} MERS-CoV in
Saudi Arabia based on index cases available from a recently-published
compendium of cases.\textsuperscript{32} For the latest event, most if
...
Haiti\textsuperscript{33} and Europe\textsuperscript{34} are not
...
Coevolution\textsuperscript{6} to provide a measure of risk accounting
for multiple processes. In fig.~\ref{fig:trivariate}, we overlapped
three components of spillover risk: viral sharing, \emph{i.e.} the
chance that two bats will share viruses overall; Local Contribution to
Beta Diversity, \emph{i.e.} the fact that a bat community is
compositionally unique compared to the average compositional similarity
across the entire system; finally, host phylogenetic diversity,
\emph{i.e.} how dispersed the bats in a location are within the tree of
life. This approach leads to the definition of broad biogeographic
regions of risk, where the same color represents the same type of risk.
By way of constrat to figures fig.~\ref{fig:richness} and
fig.~\ref{fig:biogeo}, these regions do not necessarilly overlap with
previous spatial partitions of the bat-betacoronaviruses complex.
...
lesser degree, cospeciation and sharing.\textsuperscript{intra-genus
cross-species transmission; 2} This diversification is not an actual
risk factor for spillover itself, but acts downstream of a spillover
event by increasing the random chance of the emergence of a virus with
...
continents.\textsuperscript{\emph{e.g.} 35} Recent
surveillance\textsuperscript{36} has identified a novel
...
widely.\textsuperscript{37} Since human activity is required to amplify
...
looking for the origins of SARS in China,\textsuperscript{38} present
...
with recent results from\textsuperscript{39} -- areas of purported high
...
unsampled bat viruses,\textsuperscript{22} bat
coronaviruses,\textsuperscript{2} and even specifically
betacoronaviruses\textsuperscript{20} in both the Amazon and southeast
...
recombination (see \emph{e.g.},\textsuperscript{48} as well as the
...
lineage).\textsuperscript{49} Both of these regions are priority areas
for sampling, especially given predictions that they contain many bat
hosts of undiscovered betacoronaviruses.\textsuperscript{20,39} However,
...
to the region.\textsuperscript{16}

{[}Tim tinker with para about conservation{]} There are several factors
that drive changes in the diversity of bats,\textsuperscript{50} but
...
diseases,\textsuperscript{51} as previous studies on RNA viruses suggest
the importance of host phylogeography at the time of virus
dispersal.\textsuperscript{52} One of these scenarios where interaction
...
humans.\textsuperscript{53} This represents a challenge for conservation
...
overlap.\textsuperscript{61} This network stores pairwise values of
...
and low viral sharing,\textsuperscript{63,64} in that this allows
...
extraction,\textsuperscript{see \emph{e.g.} 65} we measured the risk
...
{[}\textsuperscript{66}; Algorithm FFT-NS-2, Scoring matrix 200PAM /
k=2, gap open penalty 1.53m offset value 0.123{]} and a maximum
likelihood tree reconstructed in IQ-TREE v 1.6.12\textsuperscript{67}
