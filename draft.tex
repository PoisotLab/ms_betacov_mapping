...
a win-win intervention for both
{[}\textsuperscript{51};;\textsuperscript{52}OHHLEP2022OneHea{]}.
...
data.\textsuperscript{53} The range maps were subsequently rasterized
using the \texttt{rasterize} function from
\texttt{GDAL}\textsuperscript{54} at a resolution of approximately
...
Diversity\textsuperscript{55} based on a recent synthetic tree with
robust time calibration, covering about 6000 mammalian
species.\textsuperscript{56} Faith's PD measures the sum of unique
...
Beta-Diversity;\textsuperscript{57} LCBD works from a species-data
...
location, and following best practices\textsuperscript{58} transformed
...
community. Recent results by\textsuperscript{59} shows that LCBD
...
mammals.\textsuperscript{60} This network stores pairwise values of
...
model.\textsuperscript{61} In this approach, every risk component gets
...
and low viral sharing,\textsuperscript{62} in that this allows multiple
...
extraction,\textsuperscript{63} we measured the risk level by
...
MAFFT\textsuperscript{64} v1.4.0 (Algorithm FFT-NS-2, Scoring matrix
200PAM / k=2, gap open penalty 1.53m offset value 0.123) and a maximum
likelihood tree reconstructed in IQ-TREE\textsuperscript{65} v1.6.12
with ModelFinder\textsuperscript{66} ultrafast bootstrap
approximation\textsuperscript{67} with a general time reversible model
...
distinctiveness\textsuperscript{68} for each of the viruses in the tree,
...
this idea, we loosely adapted a method from,\textsuperscript{69,70} who
...
cophenetic function in \texttt{ape};\textsuperscript{71} subsequently,
